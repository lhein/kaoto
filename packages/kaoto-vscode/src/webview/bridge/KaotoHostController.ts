...
interface BridgeMessage { type: string; payload: unknown; }
...
export class KaotoHostController {
  ...
  async initialize(panel: vscode.WebviewPanel, fileUri: string): Promise<void> {
    ...
    const [initialContent, initialSettings] = await Promise.all([
    ...
    this.disposables.push({ dispose: panel.onDidDispose(() => this.dispose()) });
  }
